...
None
    Result produced by this creature-domain operation."""
        # Keep reset behavior explicit in its owning subsystem.
        self.index = index
        self.count = 0
        self.generation = generation

    def invalidate(self) -> None:
        """Execute invalidate behavior.
...
None
    Result produced by this creature-domain operation."""
        # Keep invalidate behavior explicit in its owning subsystem.
        self.count = 0
        self.generation = 0
        self.index = None

    def append_slot(self, slot: int) -> None:
        """Execute append slot behavior.
...
None
    Result produced by this creature-domain operation."""
        # Keep append slot behavior explicit in its owning subsystem.
    ...
    def sort_by_stable_id(self) -> None:
        """Execute sort by stable id behavior.
...
None
    Result produced by this creature-domain operation."""
        # Keep sort by stable id behavior explicit in its owning subsystem.
    ...
    def _valid_index(self) -> CreatureSpatialIndex:
        ...
        """
        # Keep valid index behavior explicit in its owning subsystem.
        index = self.index
...
BroadPhaseGeometry
    Result produced by this creature-domain operation."""
        # Keep calculate behavior explicit in its owning subsystem.
...
class CreatureSpatialIndex:
    ...
    def _circle_values(creature: object) -> tuple[float, float, float]:
        """Execute circle values behavior.
...
tuple[float, float, float]
    Result produced by this creature-domain operation."""
        # Keep circle values behavior explicit in its owning subsystem.
    ...
    def _ensure_slot_capacity(self, capacity: int) -> None:
        """Execute ensure slot capacity behavior.
...
None
    Result produced by this creature-domain operation."""
        # Keep ensure slot capacity behavior explicit in its owning subsystem.
...
list[int]
    Result produced by this creature-domain operation."""
        # Keep claim cell behavior explicit in its owning subsystem.
    ...
    def _append_cell_slot(self, key: tuple[int, int], slot: int) -> None:
        """Execute append cell slot behavior.
...
None
    Result produced by this creature-domain operation."""
        # Keep append cell slot behavior explicit in its owning subsystem.
    ...
    def _append_family_slot(self, parent_id: int, slot: int) -> None:
        """Execute append family slot behavior.
...
None
    Result produced by this creature-domain operation."""
        # Keep append family slot behavior explicit in its owning subsystem.
    ...
    def _reset_active_storage(self) -> None:
        """Execute reset active storage behavior.
...
None
    Result produced by this creature-domain operation."""
        # Keep reset active storage behavior explicit in its owning subsystem.
    ...
    def rebuild(self, creatures: Sequence[object]) -> None:
        ...
        ValueError
            If an input or restored value violates validation rules.
        """
        # Keep rebuild behavior explicit in its owning subsystem.
    ...
    def query_into(
        ...
        RuntimeError
            If runtime state violates the callable invariant.
        """
        # Keep query into behavior explicit in its owning subsystem.
        if not self.valid:
    ...
    def creature_for_slot(self, slot: int) -> object | None:
        """Execute creature for slot behavior.
...
object | None
    Result produced by this creature-domain operation."""
        # Keep creature for slot behavior explicit in its owning subsystem.
        if (
            not self.valid
            or slot < 0
...
tuple[float, float, float] | None
    Result produced by this creature-domain operation."""
        # Keep values for behavior explicit in its owning subsystem.
        stable_id = getattr(creature, "creature_id", None)
...
FamilyView
    Result produced by this creature-domain operation."""
        # Keep family view behavior explicit in its owning subsystem.
